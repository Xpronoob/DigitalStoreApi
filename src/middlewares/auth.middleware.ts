...
export class AuthMiddleware {
  ...
  static authorization = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const payloadAccessToken = await JwtAdapter.validateAccessToken<{ user_id: string, expired: boolean }>(accessToken)
      const payloadRefreshToken = await JwtAdapter.validateRefreshToken<{ user_id: string, expired: boolean }>(refreshToken)
