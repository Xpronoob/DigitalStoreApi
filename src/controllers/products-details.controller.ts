...
export class ProductDetailsController {
  ...
  create = async (req: Request, res: Response) => {
    try {
      const validatedData = ZodProductDetailsAdapter.validateProductDetail(
        req.body,
      )
      const productDetail =
        await this.productDetailsRepository.create(validatedData)

      if (!productDetail)
        throw CustomError.internalServer(
          'Error al crear el detalle del producto',
        )
  ...
  update = async (req: Request, res: Response) => {
    try {
      ...
      const updatedProductDetail = await this.productDetailsRepository.update(
  ...
  delete = async (req: Request, res: Response) => {
    try {
      const deletedProductDetail = await this.productDetailsRepository.delete(
        parseInt(req.params.id),
      )
  ...
  getById = async (req: Request, res: Response) => {
    try {
      const productDetail = await this.productDetailsRepository.getById(
        parseInt(req.params.id),
      )
  ...
  toggleStatus = async (req: Request, res: Response) => {
    try {
      ...
      const updatedProductDetail =
        await this.productDetailsRepository.toggleStatus(
      ...
      if (!updatedProductDetail)
        throw CustomError.internalServer(
          'Error al actualizar el estado del detalle del producto',
        )

      res.status(200).json({
        message: active
          ? 'Detalle del producto activado exitosamente'
          : 'Detalle del producto desactivado exitosamente',
        updatedProductDetail,
      })
    } catch (error) {
      if (error instanceof CustomError) {
